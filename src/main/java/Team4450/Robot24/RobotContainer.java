...
public class RobotContainer {
  ...
  public RobotContainer() throws Exception {
    ...
    if (navx.isConnected()) Util.consoleLog("NavX version=%s", navx.getAHRS().getFirmwareVersion());
    else {
   ...
   *
   * <p>These buttons are for real robot driver station with 3 sticks and launchpad. The launchpad
                ...
                () ->
                    PointToYaw.yawFromAxes(
                        -MathUtil.applyDeadband(
                            driverController.getRightX(), Constants.DRIVE_DEADBAND),
                        -MathUtil.applyDeadband(
                            driverController.getRightY(), Constants.DRIVE_DEADBAND)),
                driveBase,
                false));
  ...
  public void getMatchInformation() {
    ...
    Util.consoleLog(
        "Alliance=%s, Location=%d, FMS=%b event=%s match=%d msg=%s",
        alliance.name(),
        location,
