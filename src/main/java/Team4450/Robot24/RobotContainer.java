...
public class RobotContainer {
	...
	// private PowerDistribution pdp = new PowerDistribution(REV_PDB,
	...
	private enum AutoProgram {
		NoProgram, DriveOut, TestAuto1, TestAuto3
	}
	...
	public RobotContainer() throws Exception {
		...
		// Create NavX object here since must done before CameraFeed is created (don't remember
		// why).
		...
		driveBase.setDefaultCommand(new DriveCommand(driveBase, driverController.getLeftYDS(),
				driverController.getLeftXDS(), driverController.getRightXDS(), driverController));
		...
		// being done while we are getting started up. Hopefully will complete before we are ready
		// to
		// use the trajectory.
	 ...
	 * 
	 * These buttons are for real robot driver station with 3 sticks and launchpad. The launchpad
	 * makes the colored buttons look like a joystick.
	 */
	private void configureButtonBindings() {
		...
		new Trigger(() -> driverController.getPOV() != -1).onTrue(new PointToYaw(
				() -> PointToYaw.yawFromPOV(driverController.getPOV()), driveBase, true));
		...
		// toggle slow-mode
		new Trigger(() -> driverController.getLeftTrigger()).whileTrue(
				new StartEndCommand(driveBase::enableSlowMode, driveBase::disableSlowMode));
		...
		// watchdogs (20ms). Threading does not as the toggle method is not run on the scheduler
		// thread.
		// Also, any action that operates air valves, there is a 50ms delay in the ValveDA and SA
		// classes to apply power long enough so that the valve slides move far enough to
		// open/close.
		...
		// Note: the threaded command can only execute a runable (function on a class) not a
		// Command.
	...
	/**
	 * Use this to pass the autonomous command(s) to the main {@link Robot} class. Determines which
	 * auto command from the selection made by the operator on the DS drop down list of commands.
	 * 
	...
	// //for (Integer i = 1; i < STARTING_POSES.length; i++)
	// startingPoseChooser.addOption(i.toString(), i);
	...
	public void resetFaults() {
		// This code turns on/off the automatic compressor management if requested by DS. Putting
		// this
		...
		if (monitorPDPThread != null)
			monitorPDPThread.reset();
	}
	 ...
	 * 
	 * @param fileName Name of file. Will automatically look in deploy directory and add the .path
	 *        ext.
	 * @return The path's trajectory.
	 */
	...
	// trajectoryFilePath = Filesystem.getDeployDirectory().toPath().resolve("pathplanner/" +
	// fileName + ".path");
