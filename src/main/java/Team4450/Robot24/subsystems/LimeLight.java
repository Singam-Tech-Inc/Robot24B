...
public class LimeLight extends SubsystemBase {
  ...
  public void saveSnapshots(boolean save) {
    if (save) snapshot.setDouble(1);
    else snapshot.setDouble(0);
  }
  ...
  /**
   * Update the targeting information held in this class from the LimeLight via the network tables.
   *
  ...
  public boolean processImage() {
    ...
    if (targetVisible != 0) targetFound = true;
  ...
  private int deNormalizeScreenSpaceX(double value) {
    ...
    if (value >= 0) pixels += xResolution / 2;
    else pixels = (xResolution / 2) - pixels;
  ...
  private int deNormalizeScreenSpaceY(double value) {
    ...
    if (value >= 0) pixels += yResolution / 2;
    else pixels = (yResolution / 2) - pixels;
   ...
   *     calibrated for the correct target distance. Will return zero when at correct distance to
   *     target.
   */
