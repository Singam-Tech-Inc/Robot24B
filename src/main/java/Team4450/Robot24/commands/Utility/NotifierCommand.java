 ...
 * no end condition if period != 0; either subclass it or use {@link Command#withTimeout(double)} or
...
public class NotifierCommand extends Command {
  ...
  /**
   * Creates a new NotifierCommand. Caller must schedule the returned command for it to execute.
   *
  ...
  /**
   * Creates a new NotifierCommand. Caller must schedule the command for it to execute.
   *
   * @param toRun The runnable for the notifier to run.
   * @param period The period at which the notifier should run, in seconds. Zero runs the runnable
   *     one time then the command ends.
