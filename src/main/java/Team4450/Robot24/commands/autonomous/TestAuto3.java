...
/**
 * This is an example autonomous command based on 4450 customized version of Wpilib Trajetory
 * following commands.
 */
public class TestAuto3 extends Command {
	...
	/**
	 * Creates a new TestAuto3 autonomous command. This command demonstrates one possible structure
	 * for an autonomous command and shows the use of the autonomous driving support commands.
	 *
	...
	public void initialize() {
		...
		LCD.printLine(LCD_1, "Mode: Auto - TestAuto3 - All=%s, Location=%d, FMS=%b, msg=%s",
				alliance.name(), location, DriverStation.isFMSAttached(), gameMessage);
		...
		Trajectory exampleTrajectory = TrajectoryGenerator.generateTrajectory(
				...
				List.of(new Translation2d(startingPose.getX() + 1, startingPose.getY() + 1),
						new Translation2d(startingPose.getX() + 1, startingPose.getY() - 1)),
				// End 3 meters straight ahead of where we started, facing forward
				new Pose2d(startingPose.getX() + 3, startingPose.getY(),
		...
		// new Pose2d(startingPose.getX(), startingPose.getY() + 2.0, new
		...
		// command = new AutoDriveTrajectory(driveBase, RobotContainer.testTrajectory,
		// StopMotors.stop, Brakes.on);
		// command = new AutoDriveTrajectory(driveBase, RobotContainer.ppTestTrajectory,
		// StopMotors.stop, Brakes.on);
	...
	/**
	 * Returns true when this command should end. That should be when all the commands in the
	 * command list have finished.
	 */
	@Override
