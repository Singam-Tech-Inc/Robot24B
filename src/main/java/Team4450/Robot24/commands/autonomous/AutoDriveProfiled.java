...
/**
 * A command that will drive the robot forward/backward to the specified distance using a motion
...
public class AutoDriveProfiled extends ProfiledPIDCommand {
    ...
    public void end(boolean interrupted) {
        ...
        Util.consoleLog("end: target=%.3f  actual=%.3f  error=%.2f pct  yaw=%.2f hdng=%.2f",
                Math.abs(distance), actualDist,
                (actualDist - Math.abs(distance)) / Math.abs(distance) * 100.0, driveBase.getYaw(),
