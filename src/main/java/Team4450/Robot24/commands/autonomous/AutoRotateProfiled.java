...
public class AutoRotateProfiled extends ProfiledPIDCommand {
  ...
  public AutoRotateProfiled(DriveBase driveBase, double targetAngle) {
    ...
    super(
        new ProfiledPIDController(
            kP, kI, kD, new TrapezoidProfile.Constraints(kMaxRotationVelrs, kMaxRotationAccelrss)),
  ...
  public boolean isFinished() {
    ...
    return Math.abs((getController().getGoal().position - m_measurement.getAsDouble()))
        <= kToleranceRad;
  }
  ...
  public void end(boolean interrupted) {
    ...
    Util.consoleLog(
        "hdg=%.2f  target=%.2f  yaw=%.2f  error=%.2f pct",
