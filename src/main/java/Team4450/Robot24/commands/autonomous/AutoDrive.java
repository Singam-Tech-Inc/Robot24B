...
public class AutoDrive extends Command {
   ...
   *
   * <p>Auto drive straight in current direction and power for specified distance. Stops with or
  ...
  public AutoDrive(
    ...
    Util.consoleLog(
        "spd=%.2f  tgt=%.2f  stop=%s  brakes=%s  pid=%s", speed, targetDistance, stop, brakes, pid);
  ...
  public void execute() {
    ...
    if (pid == Pid.on) {
      ...
      Util.consoleLog(
          "actdist=%.2f  error=%.2f  speed=%.3f  time=%.3f",
          actualDistance, pidController.getError(), speed, elapsedTime);
    } else Util.consoleLog("tgt=%.2f  act=%.2f", targetDistance, Math.abs(actualDistance));
  ...
  public void end(boolean interrupted) {
    ...
    Util.consoleLog(
        "target dist=%.2f  actual dist=%.2f  error=%.2f pct",
        targetDistance, actualDistance, (actualDistance - targetDistance) / targetDistance * 100.0);
