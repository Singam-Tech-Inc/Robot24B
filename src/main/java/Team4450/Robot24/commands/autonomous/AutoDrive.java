...
public class AutoDrive extends Command {
	 ...
	 * 
	 * Auto drive straight in current direction and power for specified distance. Stops with or
	 * without brakes on CAN bus drive system.
	 *
	 ...
	 * @param stop Stop stops motors at end of move, dontStop leaves power on to flow into next
	 *        move.
	 * @param brakes Brakes on or off.
	 * @param pid On is use PID to control movement, off is simple drive.
	 */
	public AutoDrive(DriveBase driveBase, double speed, double targetDistance, StopMotors stop,
			Brakes brakes, Pid pid) {
		this.driveBase = driveBase;

		Util.consoleLog("spd=%.2f  tgt=%.2f  stop=%s  brakes=%s  pid=%s", speed, targetDistance,
				stop, brakes, pid);
	...
	public void end(boolean interrupted) {
		...
		Util.consoleLog("target dist=%.2f  actual dist=%.2f  error=%.2f pct", targetDistance,
				actualDistance, (actualDistance - targetDistance) / targetDistance * 100.0);
